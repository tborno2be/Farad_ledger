...
def ingest_measurement(led: Ledger, csv_path, *, session_id, state_id, technique,
    ...
    led.finish_measurement(mid, acquisition_status=acquisition_status,
                           data_path=str(out))
    # 附件内容进库（防误删）：raw.txt / script.mscr / data.csv 的本体存 artifact 表。
    # 从此 cvlab.sqlite 自足——磁盘文件全丢也能 restore_artifacts() 完整重建。
    n_art = 0
    ...
    for kind, fname in (("raw", "raw.txt"), ("mscript", "script.mscr"),
        ...
        if p.exists():
            ...
            n_art += 1
    return {"measurement_id": mid, "parquet": str(out), "n_points": len(rows),
            "artifacts": n_art}
